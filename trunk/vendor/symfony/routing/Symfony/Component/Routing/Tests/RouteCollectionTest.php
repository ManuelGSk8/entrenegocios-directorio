...
class RouteCollectionTest extends \PHPUnit_Framework_TestCase
{
    ...
    public function testAddPrefix()
    {
        ...
        $collection->addPrefix('/{admin_user}', array('admin_user' => 'admin_user'), array('admin_user' => '\d+'));
        $this->assertEquals('/{admin_user}/foo', $collection->get('foo')->getPath(), '->addPrefix() adds a prefix to all routes');
        $this->assertEquals('/{admin_user}/bar', $collection->get('bar')->getPath(), '->addPrefix() adds a prefix to all routes');
        $this->assertEquals(array('admin_user' => 'admin_user'), $collection->get('foo')->getDefaults(), '->addPrefix() adds defaults to all routes');
        $this->assertEquals(array('admin_user' => 'admin_user'), $collection->get('bar')->getDefaults(), '->addPrefix() adds defaults to all routes');
        $this->assertEquals(array('admin_user' => '\d+'), $collection->get('foo')->getRequirements(), '->addPrefix() adds requirements to all routes');
        $this->assertEquals(array('admin_user' => '\d+'), $collection->get('bar')->getRequirements(), '->addPrefix() adds requirements to all routes');
        $collection->addPrefix('0');
        $this->assertEquals('/0/{admin_user}/foo', $collection->get('foo')->getPattern(), '->addPrefix() ensures a prefix must start with a slash and must not end with a slash');
        $collection->addPrefix('/ /');
        $this->assertSame('/ /0/{admin_user}/foo', $collection->get('foo')->getPath(), '->addPrefix() can handle spaces if desired');
        $this->assertSame('/ /0/{admin_user}/bar', $collection->get('bar')->getPath(), 'the route pattern of an added collection is in synch with the added prefix');
    }
    ...
    public function testAddPrefixOverridesDefaultsAndRequirements()
    {
        ...
        $collection->addPrefix('/admin_user', array(), array('_scheme' => 'https'));
